"""inspectord-process-collector-exit worker entry point.

Loads the sched_process_exit tracepoint via the inspectord_native Rust
extension, polls the EXIT_EVENTS ring buffer, and emits one normalized
process_exit Event per record.

Run standalone (for debugging):
  sudo python -m inspectord.workers.process_collector_exit --sink-path -
...
def _default_stream_factory() -> _StreamProtocol:
    from inspectord._native import ProcessExitStream  # noqa: PLC0415

    stream: _StreamProtocol = ProcessExitStream()
    return stream


class ProcessCollectorExitWorker:
    """Polls a ProcessExitStream and writes one Event per record.
    ...
    def _record_to_event(self, record: dict[str, Any]) -> dict[str, Any]:
        ...
        # ProcessExitStream pre-decodes the exit_code; exit_status is set for
        # normal exits, killed_by_signal is set when a signal terminated the
        # task. exit_code is the raw kernel value.
        exit_status = record.get("exit_status")
        killed_by_signal = record.get("killed_by_signal")
        outcome: str
        if exit_status == 0:
            outcome = "success"
        elif exit_status is not None or killed_by_signal is not None:
            outcome = "failure"
        else:
            outcome = "unknown"
        ...
        if exit_status is not None:
            process["exit_status"] = int(exit_status)
        if killed_by_signal is not None:
            process["killed_by_signal"] = int(killed_by_signal)
        event = build_event(
            ...
            process=process,
            raw={"source": "ebpf:sched_process_exit"},
        )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="inspectord-process-collector-exit",
        description="eBPF process-exit collector; writes NDJSON Events to a sink.",
    )
    parser.add_argument(
        "--sink-path",
        default="-",
    ...
    worker = ProcessCollectorExitWorker(sink=sink)
    worker.start()
    try:
        while True:
