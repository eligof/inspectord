"""inspectord-outbound-connection-tracker6 worker entry point.

IPv6 sibling of the outbound_connection_tracker worker. Loads the
inet_sock_set_state tracepoint via the inspectord_native Rust extension,
polls the CONNECT6_EVENTS ring buffer, and emits one normalized
outbound_connection Event per IPv6 SYN_SENT -> ESTABLISHED transition the
kernel observes. Loopback connections (::1) are already filtered at the
native boundary.

Run standalone (for debugging):
  sudo python -m inspectord.workers.outbound_connection_tracker6 --sink-path -
...
def _default_stream_factory() -> _StreamProtocol:
    from inspectord._native import ProcessConnectStream6  # noqa: PLC0415

    stream: _StreamProtocol = ProcessConnectStream6()
    return stream


class OutboundConnectionTracker6Worker:
    """Polls a ProcessConnectStream6 and writes one Event per record.
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="inspectord-outbound-connection-tracker6",
        description="eBPF outbound IPv6 TCP connection tracker; writes NDJSON Events to a sink.",
    )
    parser.add_argument(
        "--sink-path",
        default="-",
    ...
    worker = OutboundConnectionTracker6Worker(sink=sink)
    worker.start()
    try:
        while True:
