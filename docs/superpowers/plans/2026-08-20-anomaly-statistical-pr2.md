...
- Rate metrics zero-fill: once an entity is tracked, every closed minute without activity pushes `0.0` — absence is signal for rates. Exception: after a gap larger than `_MAX_CATCHUP_MINUTES` (180, i.e. 3 h — e.g. laptop suspend), the engine skips the whole span with no zero-fill, preserving baselines at the cost of slight mean bias, and bounding catch-up work. Logged. *(Amended during review from the original 1440-with-zero-fill design, which wiped the 1h/24h rings and stalled the tick thread.)*
